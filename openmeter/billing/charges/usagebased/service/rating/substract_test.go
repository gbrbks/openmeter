...
func runSubtractRatedRunDetailsTestCases(
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertRatedRunDetailsTotalsMatchDetailedLines(t, out)
			require.Equal(t, tc.expectedTotals, toExpectedTotals(out.Totals))
			require.Equal(t, tc.expectedLines, toExpectedDetailedLines(out.DetailedLines))
		})
	}
}
...
func TestSubtractRatedRunDetailsMixedPositiveAndNegativeLines(t *testing.T) {
	...
	a.Totals = totalsFromExpectedDetailedLines(toExpectedDetailedLines(a.DetailedLines))
	...
	b.Totals = totalsFromExpectedDetailedLines(toExpectedDetailedLines(b.DetailedLines))
	...
	}, toExpectedDetailedLines(out.DetailedLines))
}

func rateRunDetailsForTest(t *testing.T, ratingService billingrating.Service, in rateRunDetailsForTestInput, opts ...billingrating.GenerateDetailedLinesOption) RatedRunDetails {
	t.Helper()
	...
	return RatedRunDetails{
		Totals:        res.Totals,
		DetailedLines: usageBasedDetailedLinesForTest(res.DetailedLines, in.ServicePeriod),
	}
}
...
func assertRatedRunDetailsTotalsMatchDetailedLines(t *testing.T, out RatedRunDetails) {
	t.Helper()

	require.Equal(t, toExpectedTotals(totalsFromExpectedDetailedLines(toExpectedDetailedLines(out.DetailedLines))), toExpectedTotals(out.Totals))
}
